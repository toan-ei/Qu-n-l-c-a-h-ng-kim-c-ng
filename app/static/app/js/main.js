...
var staticUrl = "{% static '' %}";  // Đặt URL tĩnh

function showProducts(collection) {
    // Ẩn các bộ sưu tập hiện tại
    document.querySelector('.collection_section').style.display = 'none';

    // Hiện phần grid sản phẩm
    const productGrid = document.getElementById('product_grid');
    productGrid.style.display = 'block';

    // Tạo danh sách sản phẩm tương ứng với bộ sưu tập
    let products = [];
    if (collection === 'rings') {
        products = [
            { name: "Nhẫn 1", price: "Rs. 50,000", image: staticUrl + 'app/image/25.png' },
            { name: "Nhẫn 2", price: "Rs. 55,000", image: staticUrl + 'app/image/27.png' }
        ];
    } else if (collection === 'earrings') {
        products = [
            { name: "Bông Tai 1", price: "Rs. 30,000", image: staticUrl + 'app/image/28.png' },
            { name: "Bông Tai 2", price: "Rs. 35,000", image: staticUrl + 'app/image/29.png' }
        ];
    } else if (collection === 'bracelets') {
        products = [
            { name: "Vòng Tay 1", price: "Rs. 40,000", image: staticUrl + 'app/image/30.png' },
            { name: "Vòng Tay 2", price: "Rs. 45,000", image: staticUrl + 'app/image/31.png' }
        ];
    }

    // Thêm sản phẩm vào grid
    const productItems = document.getElementById('product_items');
    productItems.innerHTML = ''; // Xóa nội dung cũ
